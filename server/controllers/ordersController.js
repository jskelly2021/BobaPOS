...
export const addOrderItemTopping = async (req, res) => {
    ...
    try {
        const result = await pool.query(`INSERT INTO order_item_topping (order_item_id, topping_id , topping_quantity)
                                        VALUES ($1, $2, $3) RETURNING *`, [order_item_id, topping_id , topping_quantity]);
        res.status(201).json(result.rows);
        console.log(`Inserting Order Item Toppings`);
    }
    catch (err) {
